...
class C(BaseConstants):
    NAME_IN_URL = "ch9_matching_ia"
    PLAYERS_PER_GROUP = 6
    NUM_ROUNDS = 1

    MATCHING = "IA"
    INSTRUCTIONS_TEMPLATE = "ch9_matching_ia/instructions.html"
...
def ia_algorithm(
    ...
    print("IAIAIAIAIAIAIAIAIAIAIAIAIAIAIAIAIAIAIAIAIA")
    ...
    for s in C.student:
        if totyu_studentStatus[s] is None:  # 相手が決まっていない学生がいれば・・・
            ...
            if len(totyu_teacherStatus[t]) < C.capacity_lim[t]:  # 定員に空きがあれば・・・
                ...
                and teiinflg[t] != "shime"
            ):  # 定員がいっぱいなら#ボストン用追加：定員が一杯フラグが違うのであれば
            ...
            # ボストン用追加：学生を1順したら、定員を締める：
            print(totyu_studentStatus, s, student_choices1[s])
            if s == "student6":
                for t in C.teacher:
                    if (
                        len(totyu_teacherStatus[t]) == C.capacity_lim[t]
